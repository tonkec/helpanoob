...
class PostsController < ApplicationController
  ...
  def create
    ...
    if @post.save(post_params)
      ...
      #PostsWorker.perform_in(30.seconds, User.first.id, @post.id)
      @users.each do |user|
        PostsWorker.perform_in(30.seconds, user.id, @post.id)
      end
    else
      render "new"
    end
  ...
  private
    ...
    def post_params
      params.require(:post).permit(:description, :title, :tag_list, :bootsy_image_gallery_id, {images: []})
    end
